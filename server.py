...
def init():
    ...
    if not c.execute("SELECT id FROM users LIMIT 1").fetchone():
        ...
        c.executemany(
            "INSERT INTO clients(user_id,name,email,phone,sector) VALUES(?,?,?,?,?)",
            [
        ...
        c.executemany(
            "INSERT INTO interventions(user_id,client,title,date,status) VALUES(?,?,?,?,?)",
            [
...
def dashboard():
    ...
    out = {
        "clients": c.execute(
            "SELECT COUNT(*) n FROM clients WHERE user_id=?",
            (uid,)
        ).fetchone()["n"],

        "quotes": c.execute(
            "SELECT COUNT(*) n FROM quotes WHERE user_id=?",
            (uid,)
        ).fetchone()["n"],

        "pending_quotes": c.execute(
            "SELECT COUNT(*) n FROM quotes WHERE user_id=? AND status='À valider'",
            (uid,)
        ).fetchone()["n"],

        "interventions": c.execute(
            "SELECT COUNT(*) n FROM interventions WHERE user_id=?",
            (uid,)
        ).fetchone()["n"],

        "documents": c.execute(
            "SELECT COUNT(*) n FROM documents WHERE user_id=? AND status!='Traité'",
            (uid,)
        ).fetchone()["n"]
    }
...
@app.get("/api/quotes")
def quotes():
    return list_rows("quotes")

@app.get("/api/interventions")
def interventions():
    return list_rows("interventions")

@app.get("/api/documents")
def documents():
    return list_rows("documents")
...
def add_intervention():
    ...
    c.execute(
        ...
        (
            uid,
            d.get("client", ""),
            d.get("title", "Nouvelle intervention"),
            d.get("date"),
            d.get("status", "À planifier")
        )
    )
...
def ai():
    ...
    if not os.environ.get("OPENAI_API_KEY"):
        return jsonify(
            answer="Mode démo : j'ai analysé la demande. Je peux préparer le client, le devis, les tâches et les prochaines actions dès que la clé IA est configurée."
        )
    ...
    try:
        ...
        return jsonify(
            answer="L'agent IA est temporairement indisponible. Vérifie la configuration API.",
            detail=str(e)
        ), 200
